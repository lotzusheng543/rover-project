...
Servo myservo; 
int servoPin = 13;
CytronMD motor1(PWM_DIR, 21, 19);
CytronMD motor2(PWM_DIR, 33, 32);
const char* ssid = "Group5Rover";
const char* password = "12345678";
WiFiServer server(80);
int manualSpeed = 10;  // Default speed
bool isMoving = false;  // Track whether motors are moving
...
void loop() {
  ...
  if (client) {
    ...
    // Handle movement based on command
    if (request.indexOf("GET /L") != -1) {
      Serial.println("Turning Left");
      myservo.write(30);
      isMoving = true;                   // Motors are now moving
      ...
      motor1.setSpeed(-manualSpeed*1.4);      // Both motors at current speed (forward or backward based on sign)
      motor2.setSpeed(-manualSpeed*2);
      isMoving = true;                   // Motors are now moving
    } else if (request.indexOf("GET /O") != -1) {
      Serial.println("Stopping");
      motor1.setSpeed(0);
      motor2.setSpeed(0);
      isMoving = false;                  // Motors are now stopped
    }else if (request.indexOf("GET /A") != -1) {
    ...
    if (request.indexOf("GET /speed?value=") != -1) {
      ...
      String valueStr = request.substring(idx);
      int endIdx = valueStr.indexOf(" ");
      if (endIdx != -1) valueStr = valueStr.substring(0, endIdx);
      int speed = valueStr.toInt();
      Serial.print("Setting speed to: ");
      Serial.println(speed);
      manualSpeed = speed;  // Store the signed speed value

      // Update motor speeds if currently moving
      if (isMoving) {
        motor1.setSpeed(-manualSpeed*1.4);
        motor2.setSpeed(-manualSpeed*2);
      }
    }
    // Send web UI
    ...
    client.println("    }");
    client.println("    .top-section {");
    client.println("      text-align: center;");
    client.println("      margin-bottom: 30px;");
    client.println("    }");
    client.println("    .top-section img {");
    client.println("      max-width: 150px;");
    ...
    client.println("      height: 80vh;");
    client.println("    }");
    client.println("    .left-box {");
    client.println("      border: 2px solid white;");
    client.println("      padding: 50px;");
    client.println("      border-radius: 15px;");
    client.println("    }");
    client.println("    .arrow-buttons {");
    client.println("      display: flex;");
    ...
    client.println("      align-items: center;");
    client.println("    }");
    client.println("    .right-controls {");
    client.println("      display: flex;");
    client.println("      flex-direction: column;");
    client.println("      align-items: center;");
    client.println("      gap: 20px;");
    client.println("    }");
    client.println("    .speed-label {");
    ...
    client.println("      border: 3px solid #01f;");
    client.println("      background-color: #88f;");
    client.println("      color: white;");
    client.println("      cursor: pointer;");
    client.println("    }");
    client.println("    .button:hover {");
    client.println("      background-color: #66c;");
    client.println("    }");
    client.println("    .stop-button {");
    client.println("      background-color: red;");
    client.println("      border: 2px solid lightgreen;");
    client.println("      color: white;");
    client.println("    }");
    ...
    client.println("    }");
    client.println("    .go-button {");
    client.println("      background-color: lightgreen;");
    client.println("      border: 2px solid white;");
    client.println("      color: black;");
    client.println("    }");
    client.println("    .stop-button:hover,");
    client.println("    .on-button:hover,");
    client.println("    .go-button:hover {");
    ...
    // Left Controls
    client.println("    <div class='left-box'>");
    client.println("      <div class='arrow-buttons'>");
    ...
    client.println("        <button class='button go-button' onclick=\"sendCommand('S')\">⬆</button>");
    client.println("        <button class='button' onclick=\"sendCommand('A')\">⇒</button>");
    client.println("        <button class='button' onclick=\"sendCommand('R')\">→</button>");
    client.println("      </div>");
    client.println("    </div>");

    // Right Controls
    client.println("    <div class='right-controls'>");
    client.println("      <div class='speed-label'>Speed</div>");
    client.println("      <p>Speed: <span id='speedValue'>10</span></p>");
    client.println("      <input type='range' class='slider' id='speedSlider' min='-122.5' max='122.5' value='0' oninput='updateSpeed(this.value)'>");
    ...
    client.println("        <button class='button on-button' onclick=\"sendCommand('F')\">Patrol</button>");
    client.println("      </div>");
    client.println("    </div>");
    client.println("  </div>");
